#%%
from codes.Module import *
from codes.pipeline import *
from codes.mltools import *
...
mse = MSELoss()
bce = BCELoss()
crossentr = Softmax_CELoss() #cross entropy avec log softmax
#%%
## Lineaire et MSE
linear.zero_grad()
...
soft_lin = softmax.forward(res_lin.reshape(-1)).reshape(-1,1)
...
res_ce = crossentr.forward(dataymulti,np.argmax(datax, axis=1))
delta_ce = crossentr.backward(dataymulti,np.argmax(datax, axis=1))
#%%
## Convolutions
linear.zero_grad()
conv1D.zero_grad()
